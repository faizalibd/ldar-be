...
exports.get = async (req, res) => {
  ...
  try {
    const result = await db.reference.role.get(
      req.params.id,
      req.query.name,
      req.query.limit,
      req.query.offset
...
exports.add = async (req, res) => {
  let response = {
    ...
    data: null,
    message: MessageProvider.message(Messages.KEYS.ADD_SUCCESS, "Role"),
  };

  try {
    response.data = await db.reference.role.add(req);
  } catch (error) {
    response.code = MessageProvider.status(Messages.KEYS.ERROR);
    response.message = `${MessageProvider.message(
      Messages.KEYS.ERROR,
...
exports.update = async (req, res) => {
  let response = {
    ...
    data: null,
    message: MessageProvider.message(Messages.KEYS.UPDATE_SUCCESS, "Role"),
  };
  try {
    response.data = await db.reference.role.update(req);
  } catch (error) {
    response.code = MessageProvider.status(Messages.KEYS.ERROR);
    response.message = `${MessageProvider.message(
...
exports.delete = async (req, res) => {
  let response = {
    ...
    data: null,
    message: MessageProvider.message(Messages.KEYS.DELETE_SUCCESS, "Role"),
  };

  try {
    await db.reference.role.delete(req);
  } catch (error) {
    response.code = MessageProvider.status(Messages.KEYS.ERROR);
    response.message = `${MessageProvider.message(
      Messages.KEYS.ERROR,
