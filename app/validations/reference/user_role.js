...
exports.add = [
  body("nik")
    ...
    .custom(async (val) => {
      ...
      found = await api.info.employee.get(val);
      if (!found) {
        throw new Error(
  ...
  body("role").custom(async (val) => {
    ...
    if (!val || (Array.isArray(val) && val.length == 0)) {
      throw new Error(
        MessageProvider.status(Messages.KEYS.NOT_EMPTY) +
          "|" +
          MessageProvider.message(Messages.KEYS.NOT_EMPTY, "Role")
      );
    }
    try {
      if (Array.isArray(val)) {
        val.forEach(async (v) => {
          ...
          found = (await db.reference.role.exists(v)) == 1;
          if (!found) {
            throw new Error("3");
          }
        });
...
exports.delete = [
  body("nik")
    ...
    .custom(async (val, { req }) => {
      ...
      found = (await db.reference.user_role.exists(val)) >= 1;
      if (!found) {
        throw new Error(
          MessageProvider.status(Messages.KEYS.NOT_FOUND) +
            "|" +
            MessageProvider.message(Messages.KEYS.NOT_FOUND, "User Role")
        );
      }
